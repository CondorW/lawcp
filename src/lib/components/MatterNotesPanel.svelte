<script lang="ts">
    ...
    // SVELTE 5: Lokaler Zustand wird explizit mit $state deklariert
    ...
    // SVELTE 5: $effect ersetzt $: und onMount/subscribe für Seiteneffekte
    $effect(() => {
        // Wir lesen den Svelte 4 Store reaktiv aus
        const newRef = $activeMatterStore;
        
        // Wenn sich die Referenz ändert, reagieren wir
        if (newRef !== activeRef) {
            activeRef = newRef;
            if (activeRef) {
            ...
            <div class="flex-1 p-6 overflow-y-auto bg-white dark:bg-slate-900 relative">
                {#if isLoading}
                    <div class="absolute inset-0 flex flex-col items-center justify-center bg-white/80 dark:bg-slate-900/80 z-10 transition-opacity">
                        ...
                        <span class="text-sm text-slate-500 font-medium">Lade Aktennotizen aus der Cloud...</span>
                    </div>
                {/if}
                <textarea 
