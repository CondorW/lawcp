...
import type { AppData, Resource, Settings } from '$lib/types';
...
export const addTeamMember = (state: AppData, name: string, shortsign: string, color: string) => ({
	...
	settings: {
		...state.settings,
		team: [...state.settings.team, { id: uuidv4(), name, shortsign, color, isLeader: false }]
	}
});
...
export const removeTeamMember = (state: AppData, id: string) => ({
	...state,
	settings: { ...state.settings, team: state.settings.team.filter((m) => m.id !== id) }
});
...
export const addResource = (
	state: AppData,
	res: Omit<Resource, 'id' | 'created' | 'updated' | 'owner' | 'expand'>
) => ({
	...state,
	resources: [...state.resources, { id: uuidv4(), ...res }]
});
