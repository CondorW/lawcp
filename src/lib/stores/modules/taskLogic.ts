...
export const addTask = (
	...
	const newTask = TaskSchema.parse({
		...
		status: TaskStatusSchema.parse(columnId),
...
export const updateTaskRef = (state: AppData, id: string, ref: string) => ({
	...state,
	tasks: state.tasks.map((t) => (t.id === id ? { ...t, matterRef: ref } : t))
});

export const moveTask = (state: AppData, id: string, status: string) => ({
	...state,
	tasks: state.tasks.map((t) =>
		t.id === id ? { ...t, status: TaskStatusSchema.parse(status) } : t
	)
});
...
export const toggleFlag = (state: AppData, id: string, date: string | null) => ({
	...state,
	tasks: state.tasks.map((t) => (t.id === id ? { ...t, flaggedDate: date } : t))
});
...
export const toggleSubtask = (state: AppData, taskId: string, subId: string) => ({
	...
	tasks: state.tasks.map((t) =>
		t.id === taskId
			? { ...t, subtasks: recursiveUpdate(t.subtasks, subId, (s) => ({ ...s, done: !s.done })) }
			: t
	)
});

export const updateSubtaskTitle = (
	state: AppData,
	taskId: string,
	subId: string,
	title: string
	...
	tasks: state.tasks.map((t) =>
		t.id === taskId
			? { ...t, subtasks: recursiveUpdate(t.subtasks, subId, (s) => ({ ...s, title })) }
			: t
	)
});
...
export const updateSubtaskPos = (
	...
	tasks: state.tasks.map((t) =>
		t.id === taskId
			? { ...t, subtasks: t.subtasks.map((s) => (s.id === subId ? { ...s, x, y } : s)) }
			: t
	)
});
		...
		t.id === taskId
			? {
					...
					subtasks: t.subtasks.map((s) =>
						s.id === src && !s.next.includes(tgt) ? { ...s, next: [...s.next, tgt] } : s
					)
				}
			: t
	)
	...
	tasks: state.tasks.map((t) =>
		t.id === taskId
			? {
					...t,
					subtasks: t.subtasks.map((s) =>
						s.id === src ? { ...s, next: s.next.filter((n: string) => n !== tgt) } : s
					)
				}
			: t
	)
