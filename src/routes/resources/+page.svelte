    ...
    <div class="max-w-[1600px] mx-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            <div class="relative w-full sm:w-80">
                ...
                <input bind:value={filter} placeholder="Suchen..." class="w-full pl-10 pr-4 py-2.5 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-shadow text-slate-900 dark:text-white" />
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col gap-4 shadow-sm hover:shadow-md transition-shadow group">
                        <div class="flex items-start justify-between w-full gap-4">
                            <div class="flex gap-4 min-w-0">
                                <!-- BRANDING: Farben je nach Typ (Royal Blue, Gold, Ruby Red) -->
                                <div class={`p-3.5 rounded-lg shrink-0 ${
                                    res.type === 'COMPANY' ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' :
                            ...
                            <div class="flex gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity shrink-0">
                                ...
                                {#if res.type === 'COMPANY'}
                                    <button onclick={() => searchHR(res.name)} title="Im HR suchen" class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg border border-transparent hover:border-blue-100 dark:hover:bg-slate-800 dark:hover:border-slate-700">
            ...
            <div class="bg-white dark:bg-slate-900 p-6 lg:p-8 rounded-xl border border-slate-200 dark:border-slate-800 h-fit sticky top-6 shadow-sm">
                ...
                <div class="flex gap-2 mb-6 p-1 bg-slate-100 dark:bg-slate-800 rounded-lg">
                    <button onclick={() => (resType = 'COMPANY')} class={`flex-1 py-2 text-sm font-bold rounded-md transition-all ${resType === 'COMPANY' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>Firma</button>
                ...
                <div class="space-y-5">
                    <label class="block">
                        ...
                        <input bind:value={resName} class="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder={resType === 'COMPANY' ? 'Firmenwortlaut' : resType === 'AUTHORITY' ? 'Behördenname' : 'Vor- & Nachname'} />
                    </label>
                    ...
                    {#if resType !== 'AUTHORITY'}
                        <label class="block">
                            ...
                            <input bind:value={resId} class="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder={resType === 'COMPANY' ? 'z.B. FL-0002.123.456-7' : 'TT.MM.JJJJ'} />
                        </label>
                    {/if}
                    ...
                    {#if resType === 'COMPANY'}
                        <label class="block">
                            ...
                            <input bind:value={resSeat} class="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="z.B. Vaduz" />
                        </label>
                    {/if}
                    ...
                    <div class="space-y-3">
                        <label class="block">
                            ...
                            <input bind:value={resStreet} class="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Straße & Hausnr." />
                        </label>
                        <div class="flex gap-3">
                            <input bind:value={resZip} class="w-28 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="PLZ" />
                            <input bind:value={resCity} class="flex-1 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Ort" />
                        </div>
                    </div>

                    <!-- BRANDING: Save-Button in Royal Blue -->
                    <button onclick={add} disabled={!resName} class="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-white py-3 rounded-lg font-bold text-sm transition-colors mt-4 shadow-sm flex items-center justify-center gap-2">
