<script lang="ts">
	...
	import { 
		ArrowLeft, Activity, AlertCircle, CheckCircle2, Clock, ShieldAlert, Flag,
		X, ListTodo, CheckSquare, Square, Calendar
	} from 'lucide-svelte';
		...
		{#each subtasks as sub}
			<div class="flex items-start gap-3">
				<div class="mt-0.5 text-slate-400 shrink-0">
					{#if sub.done}
						<CheckSquare size={16} class="text-emerald-500" />
					{:else}
						<Square size={16} />
					{/if}
				</div>
				<div class="flex-1">
					<span class={`text-sm ${sub.done ? 'text-slate-400 line-through' : 'text-slate-700 dark:text-slate-200 font-medium'}`}>
						{sub.title}
					</span>
...
{#if isTeamLeader}
	<div class="h-screen overflow-hidden flex flex-col bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans print:bg-white print:text-black">
		...
		<div class="shrink-0 relative py-3 px-6 lg:px-8 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-center items-center min-h-[64px]">
			...
			<div class="flex items-center gap-3">
				<div class="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-md">
					...
					<span class="text-sm font-bold">{globalMetrics.totalActive} <span class="text-slate-500 font-normal">Aktiv</span></span>
				</div>
				<div class="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-md">
					<CheckCircle2 size={16} class="text-purple-500"/>
					<span class="text-sm font-bold">{globalMetrics.totalReview} <span class="text-slate-500 font-normal">Review</span></span>
				</div>
		...
		<div class="flex-1 overflow-auto custom-scrollbar p-6 lg:px-8">
			<div class="grid grid-flow-col auto-cols-[400px] gap-8 max-w-[1800px] mx-auto pb-4">
				...
				{#each userMetrics as data}
					<div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col h-max overflow-hidden">
						...
						<div class="p-5 bg-slate-50/30 dark:bg-slate-900/20">
									...
									{#each data.urgentTasks as task}
										...
										>
											...
											<div class="flex justify-between items-center mt-1 border-t border-slate-100 dark:border-slate-700 pt-2.5 w-full">
												...
												{#if task.flaggedDate}
													<div class="flex items-center gap-1.5 text-[11px] font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-0.5 rounded">
														<Flag size={12} class="fill-red-600 dark:fill-red-400" />
														GERICHTSFRIST: {new Date(task.flaggedDate).toLocaleDateString('de-DE')}
													</div>
							...
							{:else}
								<div class="text-sm text-slate-400 italic text-center py-6 flex flex-col items-center gap-2">
									<CheckCircle2 size={28} class="text-slate-300 dark:text-slate-700" />
	...
	>
		...
		>
			<div class="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/80 rounded-t-2xl flex justify-between items-start gap-4">
				...
				<button 
					onclick={() => selectedTask = null}
					class="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-full hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors"
				>
					<X size={20} />
				</button>
			</div>
				...
				<div class="flex flex-wrap gap-6 mb-8 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800">
					<div>
						...
						<div class="text-sm font-medium text-slate-700 dark:text-slate-200 flex items-center gap-2">
							...
							<span>{(selectedTask.expand?.owner as any)?.name || 'Unbekannt'}</span>
						</div>
					</div>
					
					<div>
						<div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">
							{selectedTask.flaggedDate ? 'Gerichtsfrist' : 'Target'}
						</div>
						{#if selectedTask.flaggedDate}
							<div class="text-sm font-bold text-red-600 dark:text-red-400 flex items-center gap-1.5">
								<Flag size={14} class="fill-red-600 dark:fill-red-400" />
								{new Date(selectedTask.flaggedDate).toLocaleDateString('de-DE')}
							</div>
						{:else if selectedTask.dueDate}
							<div class="text-sm font-medium text-slate-700 dark:text-slate-200 flex items-center gap-1.5">
								<Calendar size={14} class="text-slate-400" />
								{new Date(selectedTask.dueDate).toLocaleDateString('de-DE')}
							</div>
						{:else}
							<div class="text-sm text-slate-400 italic">Keine Frist</div>
						{/if}
					</div>
				</div>
