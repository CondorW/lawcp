                ...
                {#each flattenedSubs as source}
                    {#each source.next as targetId}
                        ...
                        {#if target}
                            ...
                            <path d={pathData} stroke={source.done && target.done ? "#22c55e" : "#94a3b8"} stroke-width="2" fill="none" marker-end="url(#arrow)" />
                        {/if}
                    {/each}
                {/each}
            ...
            {#each flattenedSubs as sub (sub.id)}
                <div 
                    class={`absolute w-[220px] shadow-lg z-10 group transition-colors duration-200
                        ...
                        ${sub.done ? 'bg-slate-50 dark:bg-slate-900 border-green-500/50 opacity-80 grayscale-[0.2]' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700'}
                    `}
                    style="left: {sub.x || 0}px; top: {sub.y || 0}px;"
                    onmousedown={(e) => startDrag(e, sub.id)}
                >
                    <div class={`px-3 py-2 border-b flex justify-between items-center cursor-move transition-colors
                        ${sub.level > 0 ? 'rounded-tl-3xl rounded-tr-sm' : 'rounded-t-xl'}
                        ${sub.done ? 'bg-green-50/50 dark:bg-green-900/20 border-green-100 dark:border-green-900/50' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-700'}
                    `}>
                        <div class="flex items-center gap-1.5 pl-1">
                            {#if sub.done}
                                <CheckCircle2 size={14} class="text-green-600 dark:text-green-500"/>
                            {:else if sub.level > 0}
